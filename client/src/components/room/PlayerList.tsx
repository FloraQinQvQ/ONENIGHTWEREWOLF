    ...
    <div className="space-y-2">
      {players.map(p => (
        <div key={p.userId} className="flex items-center gap-3">
          {p.avatarUrl
            ? <img src={p.avatarUrl} alt="" className="w-9 h-9 rounded-full" />
            : <div className="w-9 h-9 rounded-full bg-night-700 flex items-center justify-center font-semibold">{p.displayName[0]}</div>
          }
